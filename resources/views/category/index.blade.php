...
<div class="container pt-3 pe-2 overflow-y-auto h-dvh pb-20 md:pb-5">
    ...
    <x-tacapro.summary>
        ...
        <x-slot:todo>{{ isset($todo)? $todo : '8' }}</x-slot:todo>
        <x-slot:progress>{{ isset($progress)? $progress : '2' }}</x-slot:progress>
        <x-slot:done>{{ isset($done)? $done : '7' }}</x-slot:done>
    </x-tacapro.summary>

    {{-- Stats --}}
    <x-tacapro.stats :object="'category'">
        <x-slot:sum>{{ isset($task) ? count($task->done) : '2' }}</x-slot:sum>
        <x-slot:width>{{ isset($task) ? $task->progress . "%" : "70%" }}</x-slot:width>
    </x-tacapro.stats>

    {{-- Nav --}}
    <x-tacapro.nav :name="'category'">
        <x-slot:first>All</x-slot:first>
        <x-slot:first-value>{{ isset($category) ? count($category) : '4' }}</x-slot:first-value>

        <x-slot:second>Recently Add</x-slot:second>
        <x-slot:second-value>{{ isset($category) ? count($category->recent) : '7' }}</x-slot:second-value>

        <x-slot:third>Pin</x-slot:third>
        <x-slot:third-value>{{ isset($category) ? count($category->pin) : '2' }}</x-slot:third-value>
    </x-tacapro.nav>
