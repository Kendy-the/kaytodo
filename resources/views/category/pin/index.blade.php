<form action="/category/pin" method="post" class="bg-white flex flex-col gap-3 rounded-xl p-5 pb-3 mt-5">
    @csrf
    <input type="hidden" name="id" value="{{ $post->id }}"/>
    <div class="px-10 py-5 h-min flex flex-col gap-5">
        <div class="mt-5 text-center">
            <h1 class="my-2 text-2xl font-bold">Pin or Unpin Category</h1>
            <div>Pin or unpin my <b><i>{{ $post->name }}</i></b> category ?
            </div>
        </div>

        <div>
            <div>
                <div data-accordion-target="#accordion-collapse-body-{{ $itemId }}">
                    <x-button.primary :extend="[]" :action="'none'" :type="'submit'" :name="'submit'">
                        Yes
                    </x-button.primary>
                </div>
                <div data-accordion-target="#accordion-collapse-body-{{ $itemId }}">
                    <x-button.primary :extend="[]" :action="'none'" :type="'reset'" :name="'reset'">
                        No
                    </x-button.primary>
                </div>
            </div>
        </div>
    </div>
</form>
