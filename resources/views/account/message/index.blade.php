<x-layout.app-layout>
    ...
    <div class="container pt-3 pe-2 overflow-y-auto h-dvh pb-20 md:pb-5">
        <div class="bg-white rounded-xl p-5 mt-2">
            <h1 class="font-bold text-xl text-center my-2">messages</h1>

            {{-- S'il n'ya aucune message --}}
            @if (isset($messages))
                <div class="border-t my-1 py-3 border-gray-200 mt-5 pb-3 flex flex-col gap-3 justify-center items-center text-center">
                    <div>
                            ...
                            <g clip-path="url(#clip0_10131_1391)">
                                ...
                                <g filter="url(#filter2_dddd_10131_1391)">
                                    ...
                                    <line x1="50.166" y1="51.4443" x2="89.8327" y2="51.4443" stroke="#BDB4FE"
                                        stroke-width="2" />
                                    <line x1="50.166" y1="60.3333" x2="89.8327" y2="60.3333"
                                        stroke="#BDB4FE" stroke-width="2" />
                                    <line x1="50.166" y1="69.2222" x2="89.8327" y2="69.2222"
                                        stroke="#BDB4FE" stroke-width="2" />
                    ...
                    <div>
                        <h3 class="font-bold">No Messages Available</h3>
                        <p>
                            it looks like you don't have any message at the moment.<br>
                            This space will be updated as new message are added!
                        </p>
                    </div>
                </div>
            @else
                {{-- s'il y'a message --}}
                <a href="/account/message/show/{{ isset($message) && !empty($message) ? $message->id : 1 }}" class="flex justify-between gap-2 md:justify-around border-t my-1 py-3 border-gray-200">
                    <div>
                        @if (isset($profile_picture))
                            <img src="" alt="PP" class="rounded-full w-12 h-12">
                        @else
                            <div class="bg-violet-100 rounded-full w-13 h-13 flex justify-center items-center">
                    ...
                    <div>
                        <div class="flex justify-between">
                            <h2 class="font-bold text-lg mb-1">{{ isset($name) && !empty($name) ? $name : 'Jhon Doe'}}</h2>
                            <span>{{ isset($hour) ? $hour : '09:10' }}</span>
                        </div>
                        <div class="flex justify-between items-center gap-7">
                            <p>{{ isset($message) && !empty($message) ? substr($message,0,20) . '...' : "What's new? What do we do..."}}</p>
                            <span class="bg-violet-500 text-white rounded-full w-5 h-5 md:w-7 md:h-7 flex justify-center items-center">{{ isset($messages) && !empty($messages) ? count($messages) : '2'}}</span>
                        </div>
                    </div>
                </a>
